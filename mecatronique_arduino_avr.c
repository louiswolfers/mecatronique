...
register const unsigned long Measure_Timeout = 25000UL;
register float temperature;
register float temps; // Pas besoin d'initier une variable
register float vitesse;
register float vitesseDuSon = 0.0034; 
// On ne l'utilise qu'une fois, autant le diviser par 1000 directement
register float distance1 = 0.0;
register float distance2 = 0.0;
register float temps1 = 0.0;
register float temps2 = 0.0;
...
// Code de la librairie Arduino modifié, ne fonctionne pas
...
float conversion(uint8_t pin) {
	return (analog_read(pin) * 0.107422) - 50;
}
